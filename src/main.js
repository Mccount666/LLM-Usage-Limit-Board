...
function newProbeDiag() {
  ...
  return { statuses: [], messages: [], errors: [], responded: 0 };
}
...
function explainProbeFailure(diag, provider, mode) {
  ...
  if (notFound) {
    // Mixed failure (N-16): some candidates answered 404 while others never
    // answered at all — "所有候选路径均返回 404" would be false, and the
    // never-answered part is the one the user can act on.
    if (diag.errors.length > 0) {
      return `服务商用量接口不可用：${diag.statuses.length} 个候选返回 404、${diag.errors.length} 个无响应（网络层）。请确认 Base URL 是否指向中转站根地址，且本机网络可达`;
    }
