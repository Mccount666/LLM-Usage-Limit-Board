...
| **CherryIN** | 余额 | `https://open.cherryin.ai` | CherryIN 的 `sk-` Key | New API 栈账单对；显示剩余 Credits（USD） |
...
- **迷你状态条**：**新订阅保存且首次取数成功后自动收缩**，或随时点面板右上角 **`-`** 键收缩——屏幕右下角的一条半透明横条，每个订阅一项（`名称  5h 12.5% · 周 3.0%` 或 `名称 $12.34`），颜色随阈值变绿/黄/红。
